...
def personalDP(PID):
    ...
    if os.path.exists('PJI_model/Stacking_model_'+str(PID)) & os.path.exists("Decision_rule/decision_rule_"+str(PID)+".json"):
        ...
        rules_list = getTopN_Fidelity(
            CONDITIOS_AvgFidelity, list(explainers.keys()), 5)
        print('rules_list_ini:')
        print(rules_list)
        print('')
        rules_list = auto_pick_boundary.find_delta(rules_list, PID)
        ...
        # rules_list = auto_pick_boundary.singleton_opt(
        #     X_test, PID, final_singleton)
