...
def transPOSForm(Candidate_DecisionPath):
        ...
        if len(index) == 1:
            ...
            if (_singleton_list[index[0]].find('<=') > 0):
                for val2 in index: #index):
                    val_list.append(float(_singleton_list[val2][_singleton_list[val2].find("<=")+3:]))
                min_value = min(val_list)
                min_index = val_list.index(min_value)
                final_singleton.add(_singleton_list[index[min_index]])
                
                for i in index:
                     _singleton_list_[_singleton_list[i]] = _singleton_list[index[min_index]]
            
            elif (_singleton_list[index[0]].find('<') > 0):
                for val2 in index: #index):
                    val_list.append(float(_singleton_list[val2][_singleton_list[val2].find("<")+2:]))
                min_value = min(val_list)
                min_index = val_list.index(min_value)
                final_singleton.add(_singleton_list[index[min_index]])
                ...
                for i in index:
                     _singleton_list_[_singleton_list[i]] = _singleton_list[index[max_index]]
    ...
    a, b, c, d, e, f, g, h, i, j, k, l, m, n, o, p, q, r, s, t, u, v, w, x=symbols(sym)
    sym_array = [a, b, c, d, e, f, g, h, i, j, k, l, m, n, o, p, q, r, s, t, u, v, w, x]     
    ns = {}
...
Simplify_DecisionRules = "{}".format(simplify_logic(SOPform(ns_all, minterms_), 'dnf'))
