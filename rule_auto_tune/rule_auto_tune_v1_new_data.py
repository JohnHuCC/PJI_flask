...
def getTopN_Fidelity(fidelity_list, top_N_indices, top_N, PID):
    ...
    with open('final_result/decision_tree_v1_top'+str(top)+'.txt', 'a') as f:
...
def run_test(PID, top_num, singleton_num):
    ...
    with open('final_result/final_result_v1_top'+str(top)+'.txt', 'a') as f:
...
if __name__ == "__main__":
    ...
    for i, top in enumerate(top_num_high):
        ...
        with open('final_result/test_out_v1.txt', 'a') as f:
            f.write('Top:')
            f.write(str(top))
            # f.write('\n')
