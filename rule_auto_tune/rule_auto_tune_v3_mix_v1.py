...
import time
import auto_pick_boundary_v3_1
...
def getTopN_Fidelity(fidelity_list, top_N_indices, top_N, PID):
    ...
    with open('final_result/decision_tree_v1_top'+str(top)+'_tree.txt', 'a') as f:
...
def count_tree_fid(explainers, VAL_DATASET, Y_VAL_DATASET, tree_candidates, loaded_model, VAL_SIZE, X_test, feature_selection):
    ...
    for explain_i in list(explainers.keys()):
        ...
        d_path = {
            ...
            'Primary, Revision\nnative hip': 'Surgery',
            'Purulence': 'Purulence',  # 膿:Purulence
    ...
    return CONDITIOS_AvgFidelity


def generate_stacking_model(X_train, y_train):
...
def run_test(PID, top_num, singleton_num):
    ...
    # rules_list_temp = auto_pick_boundary_v1.singleton_opt(PID, final_singleton_ini)
    # print(rules_list_temp)
    # print('')
    rules_list = auto_pick_boundary_v3_1.find_delta(
    ...
    for i, (element) in enumerate(rules_list):
        ...
        # for atom in _singleton_set:
        #     _singleton_list.append(atom)
    final_singleton = list(_singleton_set)
    POS_Form = rules_list
    ...
    # In[21]: 列舉 all_singleton 的 features by Set()
    singleton_for_tree = set()
    for ele in final_singleton:
        eleList = ele.split(' ')
        name = eleList[0]
        singleton_for_tree.add(name)

    singleton_for_tree_list = list(singleton_for_tree)

    name_model_to_data = {
        'two_positive_culture': '2X positive culture',
        'APTT': 'APTT',
        'ASA_2': 'ASA_2',
        'Age': 'Age',
        'Hb': 'HGB',
        'P_T': 'P.T',
        'PLATELET': 'PLATELET',
        'Positive_Histology': 'Positive Histology',
        ...
        'Serum_CRP': 'Serum CRP',
        'Serum_ESR': 'Serum ESR',
        'Serum_WBC_': 'Serum WBC ',
        'Single_Positive_culture': 'Single Positive culture',
        'Synovial_WBC': 'Synovial WBC',
        'Synovial_PMN': 'Synovial_PMN',
        'Total_CCI': 'Total CCI',
        'Total_Elixhauser_Groups_per_record': 'Total Elixhauser Groups per record',
    }
    singleton_for_tree_list_tr = list()
    for i in singleton_for_tree_list:
        singleton_for_tree_list_tr.append(name_model_to_data[i])
    print('singleton_for_tree_list_tr:', singleton_for_tree_list_tr)
    # X_train_tree, y_train_tree, X_test_tree = X_train.rename(
    #     columns=d_path), y_train, X_test.rename(
    #     columns=d_path)

    X_train_tree, X_test_tree, y_train_tree = X_train[
        singleton_for_tree_list_tr], X_test[singleton_for_tree_list_tr], y_train
    X_tr_tree, X_val_tree, y_tr_tree, y_val_tree = train_test_split(
        X_train_tree, y_train_tree, test_size=0.2, random_state=666, shuffle=True)
    VAL_DATASET_tree = []
    Y_VAL_DATASET_tree = []
    for i in range(VAL_SIZE):
        VAL_DATASET_tree.append(resample(X_val_tree, n_samples=53,
                                         replace=False, random_state=i))
        Y_VAL_DATASET_tree.append(resample(y_val_tree, n_samples=53,
                                           replace=False, random_state=i))

    Stacking_model_final = generate_stacking_model(X_train_tree, y_train_tree)
    explainers_final_tree, tree_candidates_final = getCandidate(X_train_tree, y_train_tree,
                                                                X_test_tree, Stacking_model_final,
                                                                Explainer_depth, explainers_counter)
    CONDITIOS_AvgFidelity_final = count_tree_fid(
        explainers_final_tree, VAL_DATASET_tree, Y_VAL_DATASET_tree, tree_candidates_final, Stacking_model_final, VAL_SIZE, X_test_tree, singleton_for_tree_list_tr)
    with open("Tree_Candidate/Tree_candidate_new_data_"+str(PID)+"_final_tree_v3_"+str(top_num), "wb") as Tree_candidate_file:
        pickle.dump(CONDITIOS_AvgFidelity_final, Tree_candidate_file)
    rules_list_final = getTopN_Fidelity(
        CONDITIOS_AvgFidelity_final, list(explainers_final_tree.keys()), 10, PID)
    print('rules_list_final:', rules_list_final)
    rules_list_p = rules_list_final
    ...
    with open('final_result/final_result_v3_top'+str(top)+'_tree.txt', 'a') as f:
        f.write('final singleton:')
        f.write(str(singleton_for_tree_list_tr))
        f.write('\n')
        f.write('final singleton num:')
        f.write(str(len(singleton_for_tree_list_tr)))
        f.write('\n')
        f.write('CONDITIOS_AvgFidelity_final:')
        f.write(str(CONDITIOS_AvgFidelity_final))
        f.write('\n')
...
if __name__ == "__main__":
    ...
    top_num_high = [18, 19, 20, 12, 13, 14, 15, 16, 17, 7, 8, 9, 10, 11]
    ...
    for i, top in enumerate(top_num_high):
        ...
        with open('final_result/test_out_v3_tree.txt', 'a') as f:
            f.write('Top:')
            f.write(str(top))
            # f.write('\n')
