...
def transPOSForm(Candidate_DecisionPath):
    ...
    for i, (val) in enumerate(singleton_f):
        ...
        if len(index) == 1:
            ...
            eleList = _singleton_list[index[0]].split(' ')
            val = float(eleList[2])
...
def run_test(PID, top_num):
    ...
    # print('')
    rules_list = auto_pick_boundary_v3.find_delta(rules_list, PID)
    ...
    with open('final_result/final_result_v3_top'+str(top)+'.txt', 'a') as f:
...
if __name__ == "__main__":
    rule_problem = [3971, 6101, 7331, 7451, 4562, 3661, ]
    ...
    for top in top_num_high:
        ...
        with open('final_result/test_out_v3.txt', 'a') as f:
            f.write('Top:')
            f.write(str(top))
            f.write('\n')
