...
def run_test(PID, top_num, singleton_num):
    ...
    # rule_str_bound = auto_pick_boundary_v1.simplified_singleton_opt(
    #     PID, rule_str_sp)
    # print('boundary_rule:')
    ...
    for explain_i in list(explainers.keys()):
        ...
        for val_df in VAL_DATASET:
            ...
            for tree_idx in tree_candidates[explain_i]:
                fidelity.append(accuracy_score(stack_pred, merge_pred))
        AVG_FIDELITYS_kmap.append(round(np.mean(fidelity), 3))
    avg = np.mean(AVG_FIDELITYS)
    avg_kmap = np.mean(AVG_FIDELITYS_kmap)
    print(avg)
    print(avg_kmap)
    fid_all.append(avg)
    fid_all_kmap.append(avg_kmap)
    if avg < 0.5:
        problem_id.append(PID)
        fid_problem.append(avg)
    else:
        fid_list.append(np.mean(avg))
    with open('final_result/final_result_v1_top'+str(top)+'_kmap.txt', 'a') as f:
        ...
        f.write('\n')
        f.write('AVG fidelitys kmap:')
        f.write(str(avg_kmap))
        f.write('\n')
        f.write("kmap time = {}".format(kmap_time))
        f.write('\n')
    f.close()
...
if __name__ == "__main__":
    ...
    top_num_high = [12, 13, 14, 15, 16, 17, 18, 19, 20, 7, 8, 9, 10, 11]
    singleton_num = [10, 10, 10, 10, 10, 10, 12, 12, 12, 6, 6, 6, 8, 8]
    ...
    for i, top in enumerate(top_num_high):
        ...
        avg_all_kmap = numpy.mean(fid_all_kmap)
        # final_singleton_num_avg = numpy.mean(final_singleton_num)
        with open('final_result/test_out_v1_kmap.txt', 'a') as f:
            f.write('Top:')
            f.write(str(top))
            # f.write('\n')
            ...
            f.write(str(avg_all))
            f.write('\n')
            f.write('AVG_FIDELITYS_all_all_kmap:')
            f.write(str(avg_all_kmap))
            f.write('\n')
            f.write('Problem_id:')
