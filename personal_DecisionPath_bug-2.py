...
THRESHOLD = 156  # for missing rate: 200 / 323 = 0.619

# 因資料總數已減少, 應下修 threshold = 156
...
temp_col = [
    ...
    'Total Score', '2nd ICM',
    '1st ICM',
    'MSIS final classification',
    'Minor ICM Criteria Total',
    'Minor MSIS Criteria Total',
]
...
for i in range(VAL_SIZE):
    VAL_DATASET.append(resample(X_val, n_samples=len(X_val),
                       replace=False, random_state=i))
    Y_VAL_DATASET.append(resample(y_val, n_samples=len(X_val),
                         replace=False, random_state=i))
